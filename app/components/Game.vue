<template>
  <div class="game">
    ...
    <h1> {{ chapter.title}} </h1>
    <p v-for="text in level.texts" :key="text.id"> {{ text }}</p>

    <!--  on click, we change the route path-->
    <div v-for="action in level.actions" class="button" :key="action.name" @click="choiceSelected(action)" > 
      {{ action.name }} 
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    findLevel() {
      return this.findChapter().levels.find(level => level.id === parseInt(this.$route.params.id));
    },
